...
| Íconos de Phosphor dúo en vez de Lucide, y el logo real de WhatsApp (Simple Icons) en blanco sobre el azul, y en cian en el pie | Pedido del usuario (2026-09-24), elegido entre 6 sets en una página de comparación. Lucide no tiene logos de marca, y su trazo fino se veía básico. `Icono.astro` arma el SVG al compilar con `@iconify-json/ph` y `@iconify-json/simple-icons` (dependencias de desarrollo), sin JS ni CDN; si un nombre no existe, el build falla. La skill de marca sigue diciendo Lucide: acá manda el pedido |
